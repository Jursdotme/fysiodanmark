...
function secondthought_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
      ...
      if (current_user_can('activate_plugins')) {
        wp_register_script('jquery', get_template_directory_uri() . '/javascripts/lib/jquery-2.1.4.min.js', array(), '1.0.0', false); // Custom scripts
        wp_enqueue_script('jquery'); // Enqueue it!
      } else {
        wp_register_script('jquery', "http" . ($_SERVER['SERVER_PORT'] == 443 ? "s" : "") . "://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js", array(), '2.1.4', false); // jQuery
        wp_enqueue_script('jquery'); // Enqueue it!
      }
      ...
      wp_enqueue_script('modernizr'); // Enqueue it!
        ...
        wp_register_script('secondthoughtscripts', get_template_directory_uri() . '/build/scripts/all.min.js', 'jquery', '1.0.0', true); // Custom scripts
...
function secondthought_styles()
{
  ...
  wp_register_style('awesomefonts', '//maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css', array(), '1.0', 'all');
